...
def intro(monster, weapon):
    ...
    ####
    message_list = ["You find yourself standing in an open field, filled with grass and yellow wildflowers.",
        f"Rumor has it that a {monster} is somewhere around here, and has been terrifying the nearby village.", 
        "In front of you is a house.", 
        "To your right is a dark cave.", f"In your hand you hold your trusty (but not very effective) {weapon}\n"]
...
def take_action(monster, weapon, has_magic_sward):
    ...
    ####
    reflection_list_house = [["You do your best...",
        ...
        f"The sword of Ogoroth shines brightly in your hand as you brace yourself for the attack.",
        f"But the {monster} takes one look at your shiny new toy and runs away!",
    ...
    if validate(action+1, choices_number=2):
        if action == 1:
                if has_magic_sward[:1]:
                   print_pause_messages(reflection_list_house[1]) 
                else:    
                    print_pause_messages(reflection_list_house[0])
        elif action == 2:
...
def house(monster, weapon, has_magic_sward):
    ...
    house_events = ["You approach the door of the house.",
        f"You are about to knock when the door opens and out steps a {monster}.",
...
def cave(monster, weapon, has_magic_sward):
    ...
    message_list = ["You peer cautiously into the cave.",
            ...
            f"You discard your silly old {monster} and take the {weapon} with you.",
...
def where_to_go(monster, weapon, has_magic_sward):
    ...
    ####
    choice_list = ["Enter 1 to knock on the door of the house.","Enter 2 to peer into the cave."]
    print_pause_messages(choice_list)
    respond= int(input("(Please enter 1 or 2.)\n"))
    if validate(respond,len(choice_list)):
        print_pause(choice_list[respond-1][choice_list[respond-1].find("o ")+2:])
        if respond == 1:
...
def play_now():
    ...
    ## start the game
    ####
    monster_list = ["Cannibals", "lion", "bear", "tiger", "wolf", "wolfman", "vampire"]
